...
fn is_ngram_scalar_params(params: &dyn IndexParams) -> bool {
    params
        .as_any()
        .downcast_ref::<ScalarIndexParams>()
        .is_some_and(|p| p.index_type.eq_ignore_ascii_case("ngram"))
}

/// Scalar index types that build segmented (canonical per-fragment) indices and
/// therefore must let Lance generate segment UUIDs during distributed builds.
fn is_segmented_scalar(index_type: IndexType, params: &dyn IndexParams) -> bool {
    matches!(index_type, IndexType::BTree | IndexType::NGram)
        || is_btree_scalar_params(params)
        || is_ngram_scalar_params(params)
}
...
fn ensure_index_uuid_allowed(
    ...
    if index_uuid.is_some()
        ...
        && is_segmented_scalar(index_type, params)
    {
        return Err(Error::invalid_input(
