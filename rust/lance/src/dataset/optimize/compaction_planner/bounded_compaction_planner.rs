...
use crate::dataset::optimize::{
    CandidateBin, build_compaction_candidacy, collect_metrics, finalize_candidate_bins,
    get_indices_containing_frag, load_index_fragmaps,
};
use crate::{
    Dataset,
...
#[cfg(test)]
use crate::dataset::optimize::{
    CompactionPlannerType, IgnoreRemap, commit_compaction, compact_files, plan_compaction,
};
#[cfg(test)]
    ...
    fn validate(&mut self) -> Result<()> {
        if self.max_compaction_size.is_none() && self.max_compaction_rows.is_none() {
            return Err(Error::invalid_input(
                "max_compaction_size and max_compaction_rows cannot be both None when using BoundedCompactionPlanner",
            ));
        }
        Ok(())
    }
...
impl BoundedCompactionPlanner {
    ...
    fn check_and_update_usage(
        ...
        match (max_compaction_size, max_compaction_rows) {
            (None, None) => false,
