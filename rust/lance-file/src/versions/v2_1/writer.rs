...
///
/// The concrete writer owns the v2.1 encoding composition, finish ordering,
...
impl Writer {
    ...
    /// Finish the v2.1 file and close its object writer.
    pub async fn finish(&mut self) -> Result<FileWriteSummary> {
        // The order below is the v2.1 wire contract.
    ...
    /// Return the field-id to physical-column mapping selected by v2.1.
    pub fn field_id_to_column_indices(&self) -> &[(u32, u32)] {
        self.encoding.field_id_to_column_indices()
    }
}

/// Append a self-described or mini-lance v2.1 footer to an encoded batch.
