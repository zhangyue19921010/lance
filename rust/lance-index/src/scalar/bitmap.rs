...
mod tests {
    ...
    #[tokio::test]
    async fn test_bitmap_shard_merge_many_unique_keys() {
        use arrow_array::Int32Array;
        ...
        for fragment_id in 1..=4u32 {
            let start = ((fragment_id - 1) * 256) as i32;
            let values: Vec<i32> = (start..start + 256).collect();
            let row_ids: Vec<u64> = (0..256u32)
            ...
            .unwrap();
            write_bitmap_shard(store.as_ref(), batch, &[fragment_id])
                .await
                .unwrap();
        }
        ...
        let index = BitmapIndex::load(store, None, &LanceCache::no_cache())
            .await
            .unwrap();
        assert_eq!(index.index_map.len(), 1024);
        assert!(index.null_map.is_empty());

        let sample_result = index
            .search(
                &SargableQuery::Equals(ScalarValue::Int32(Some(700))),
                &NoOpMetricsCollector,
            )
            .await
            .unwrap();
        let SearchResult::Exact(sample_rows) = sample_result else {
            panic!("Expected exact result for sample query");
        };
        let actual: Vec<u64> = sample_rows
            .true_rows()
            .row_addrs()
            .unwrap()
            .map(u64::from)
        ...
        assert_eq!(
            actual,
            vec![RowAddress::new_from_parts(3, 188).into()] as Vec<u64>
        );
    }
}
